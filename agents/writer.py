...
- 必ず返信を誘う問いかけで終わる
- フォロワー以外からのリプを引き出す設計
- 6パターンのトピックは必ず全部バラけさせる（同じ成分・同じ悩みを重複させるな）
- 季節ネタは6パターン中1つまで。残り5つは通年ネタ
- 今日のトレンド・時事ネタがあればそれを絡めた投稿を1〜2パターン含めろ（芸能・SNSで流行ってること・話題のニュースに美容を絡める）

【文字数ルール（パターン別に厳守）】
- パターン1・2：25〜45文字（超短文・刺しにいく）
- パターン3〜6：50〜80文字
...
型A. 知識暴露型：
「[事実]って[数字や根拠]。[返信誘導]？😳」
例：「日焼け止め、量足りてない人9割らしい。知ってた？😳」

型B. 行動訂正型：
「[NG行動]してる人まだいる？[正解]が正解。やってた？」
例：「洗顔後すぐ保湿しないとダメ。3分以内にやってる？」

型C. やり方暴露型：
「[方法]って[意外な事実]知ってた？[返信誘導]🙌」
例：「美容液って下から上に押し込むのが正解って知ってた？🙌」

型D. 比較二択型：
「[A]派？[B]派？[煽り一言]。どっち？」
例：「朝シャン派？夜シャン派？皮膚科的には夜が正解らしい。どっち派？」

型E. 告白本音型：
「[自分のダメ告白]だった私が[変化]。[共感誘導]？」
例：「5年間毛穴に悩んでた私が変わったの、結局は洗顔の力加減だった。同じ人いる？」

型F. ミニアンケート型：
「[質問]、手あげて。[選択肢提示]」
例：「スキンケアに月いくらかけてる？3000円以下／5000円以下／1万円超。手あげて」

【絶対条件】
- 型A〜Fの順で1つずつ、計6パターン生成
- 型1・型2（A・B）は25〜45字、型3〜6（C・D・E・F）は50〜80字
- 上記info_factが渡されていれば毎回参照して新ネタを使う
- 例文と全く同じ文は禁止。事実部分を新ネタに差し替える

6パターンをJSON配列のみで返してください：
["型A投稿", "型B投稿", "型C投稿", "型D投稿", "型E投稿", "型F投稿"]"""
