...
- アフィリエイトURL・リンク：完全禁止（本文・リプ問わず）
- engage型（有益情報×煽り短文）: 70%
- list型（保存リスト型）: 30%
- アフィ単体投稿: 停止中
- リプ戦略：3パターン生成（言い換え型/共感+一撃型/問い型）・相手の口調に合わせる
- 表示回数30未満の投稿パターンは廃止
...
# 2026-04-09 引き継ぎ・やること

## 未完了（APIエラーで止まった）

- `.claude/` フォルダのpush → `git push origin main`（GH_PATでのpushを試す）
- `agents/engage_agent.py` のpopo.biyou除外 → ENGAGE_EXCLUDEリストに追加
- CLAUDE.md lessons learned セクション追加

## 今日作ったもの（未push）

- `.claude/commands/start.md` — セッション開始時にCLAUDE.md自動読み込み
- `.claude/commands/status.md` — 稼働状況確認
- `.claude/commands/deploy-check.md` — Render確認
- `.claude/commands/writer-fix.md` — writer改善
- `.claude/rules/python.md` — Pythonルール
- `.claude/rules/git.md` — Gitルール
- `.claude/rules/threads.md` — 投稿ルール
- `.claude/agents/writer.md` — writerエージェント定義
- `.claude/agents/researcher.md` — researcherエージェント定義

## Lessons learned（今日判明）

- Render APIのcron service env-varsは空（[]）→ en- Render APIのcron service env-varsは空（[YAMLで�- Render APIのcron service env-va�YAMLパーサーと衝突する
- sync_render_env.yml- sync_render_env.yml- sync_render_env.yml- sync_render_env.yml- sync_render_env.yml- sy禁- sync_render_env.yml- sync_render_env.yml- sync_render_env.yml- sync_render_env.y�- sync_render_env.yml- sync_render_env.yml- syncセ- sync_render_env.yml- sync_rend�と
  111111111111111111111111111111111111111111111111ngage_agent.pyにpopo.biyou除外を追加

3. CLAUDE.mdのlessons learnedを正式セクションとして整理
